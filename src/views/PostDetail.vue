  ...
  <div class="post-detail-page">
    <article class="w-75 mx-auto mb-5 pb-3" v-if="currentPost">
      ...
      <div v-if="showEditArea" class="btn-group mt-5">
        <router-link type="button" class="btn btn-success" :to="{ name: 'create', query: { id: currentPost.id } }">
          编辑
        </router-link>
...
export default defineComponent({
  ...
  setup() {
    ...
    const currentPost = fetchPost(currentId)
    const currentHTML = computed(() => {
      const { content, isHTML } = currentPost
    ...
    const showEditArea = computed(() => {
      ...
      if (currentPost && currentPost.author && isLogin) {
        const postAuthor = currentPost.author
        console.log(postAuthor.id, id)
